...
// 1. Interface para os parâmetros de busca da URL (Padrão Next.js 15)
interface PageProps {
  searchParams: Promise<{ [key: string]: string | string[] | undefined }>;
}

export default async function PaginaAdmin({ searchParams }: PageProps) {
  // 2. Aguarda a Promise do searchParams resolver apenas uma vez
  const resolvedParams = await searchParams;
  
  // 3. Captura o termo de busca com segurança a partir do objeto resolvido
  ...
  // 4. Filtro para facilitar encontrar o prato a editar
  ...
  return (
    <main className="p-10 max-w-6xl mx-auto">
      <div className="flex justify-between items-center mb-8">
        ...
        {/* Link para criar novo prato */}
        <Link href="/admin/novo-prato" className="bg-green-600 text-white px-6 py-2 rounded-lg font-bold hover:bg-green-700 transition-colors">
          + Novo Prato
        </Link>
      </div>
              ...
              <tr key={prato.id} className="border-b last:border-none hover:bg-gray-50 transition-colors">
                ...
                <td className="p-4 text-right">
                  <div className="flex justify-end gap-4">
                    ...
                    {/* Componente de Exclusão com confirmação */}
                    {/* Passado como String para bater com a tipagem do BotaoExcluir que corrigimos antes */}
                    <BotaoExcluir id={String(prato.id)} />
                  </div>
                </td>
              </tr>
            ))}
